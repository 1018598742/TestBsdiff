...
Java_com_fta_skr_testbsdiff_cpp_BsPatch_bspatch(JNIEnv *env, jobject type, jstring oldApk_,
...
int patchMethod(int argc, char *argv[]) {
    ...
    */
    ...
    if (((fd = open(argv[1], O_RDONLY, 0)) < 0) ||
        ...
        ((old_ = static_cast<u_char *>(malloc(oldsize + 1))) == NULL) ||
        (lseek(fd, 0, SEEK_SET) != 0) ||
        (read(fd, old_, oldsize) != oldsize) ||
        (close(fd) == -1))
        err(1, "%s", argv[1]);
    if ((new_ = static_cast<u_char *>(malloc(newsize + 1))) == NULL) err(1, NULL);
